...
def show_home():
    ...
    gid = st.session_state.get("detail_game_id")

    if view == "detail" and gid:
    ...
    with tab1:
        ...
        with col2:
            st.markdown(f"<p style='text-align:center; font-weight:600; font-size:1rem;'>Page {page}/{total_pages}</p>", unsafe_allow_html=True)
        with col3:
