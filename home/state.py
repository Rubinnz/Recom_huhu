...
# ---------- Routing ----------
def get_view() -> str:
    # Ưu tiên query params (shareable URL), fallback session
    qp = st.query_params
    if "v" in qp:
        v = qp.get("v")
...
def scroll_to_top_after_render():
    """chèn JS sau khi render xong, cuộn tới anchor #top-anchor"""
...
def reset_page_if_filter_changed(signature: tuple):
    """signature: (tuple(genres), tuple(plats), kw)"""
