...
class NetflixStyleHybridRecommender:
    """
    Hybrid Recommender = Collaborative Filtering (SVD) + Content-Based (TF-IDF)
    """

    def __init__(self, cf_model, tfidf_vectorizer, cosine_sim, games_df, ratings_df,
                 alpha=0.6, diversity_factor=0.1):
        ...
        print(f"✅ Hybrid Model Initialized | α={alpha} | Users={len(self.all_users)} | Games={len(self.all_games)}")
    ...
    def _recommend_cold_start(self, top_n=10):
        """Recommend top popular games for new users"""
    ...
    def get_user_profile(self, user_id):
        ...
        profile = {
            "user_id": user_id,
            "total_games_played": len(user_ratings),
        ...
        profile["favorite_games"] = [
            {"name": self.game_id_to_name.get(g, "Unknown"), "rating": float(r)}
            for g, r in zip(favorites["game_id"].head(5), favorites["rating"].head(5))
        ]
